...
//
// The hardened sanitizer backend: DOMPurify running over a jsdom window. Both
...
// either package is unavailable, `createDompurifyBackend` throws and wash.ts
...
import { filterEventHandlers, type Sanitizer } from './sanitizer.js';
...
function flattenAllowList(config: SanitizeConfig): {
  tags: string[];
  attrs: string[];
} {
...
/**
 * Lazily construct a DOMPurify-backed {@link Sanitizer}. Throws if `dompurify`
 * or `jsdom` is not installed — the caller is expected to catch and fall back.
 */
export async function createDompurifyBackend(): Promise<Sanitizer> {
