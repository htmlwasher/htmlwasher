---
name: python
description: Python development guidelines and best practices. Use when writing or modifying Python code in this workspace.
---

# Python Guidelines

Standards and best practices for Python development. Follow these guidelines when writing or modifying Python code in this workspace.
...
- Use an `environment.py` file with individual methods per variable (e.g., `api_key()` for `API_KEY`, `cache_dir()` for `CACHE_DIR`)
- Co-locate all environment access in one place per package for easier mocking in tests

### Data Models

- Use Pydantic v2 for schemas, validation, and data models when a real validation need exists
- Leverage Pydantic's type validation, serialization, and configuration management
- Keep thin wrappers light — do not reach for a model where a typed function signature is clearer
...
- Mock the subprocess boundary when testing code that shells out to another process

## Implementation

When implementing Python code:
- Ensure code passes `ruff` checks and tests before committing
