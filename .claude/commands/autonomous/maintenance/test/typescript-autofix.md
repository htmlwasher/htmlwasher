...
Review `.ts` files under `htmlwasher/` and `packages/live-crawl-tester/`. Exclude `node_modules/`, `dist/`, `sources/`, and `*.test.ts` files from manual review (tests are reviewed by the test commands).
