---
description: Sync READMEs with the current trafilatura-alpha library, live-crawl-tester, and training project state
allowed-tools: Bash(*), Read(*), Edit(*), Write(*), Glob(*), Grep(*)
---

# Sync htmlwasher Repo Documentation

Ensure every README under this repo reflects the current state of its source: the `trafilatura-alpha` library API, the live-crawl-tester surface, and the training project. Public exports, option/flag lists, and behaviour descriptions must be in sync across all surfaces. Every README should describe htmlwasher as a faithful **TypeScript port of Trafilatura** with page-type-aware extraction and an ONNX page-type classifier — a content-extraction **library for Node.js**, not a scraper.

**Scope:** This command only updates files inside the `htmlwasher` repo. The `trafilatura-alpha/README.md` is the published npm package page (alpha); keep it written for that audience. The root, training, and live-crawl-tester READMEs are repo-internal developer docs.
...
- **Library public API** — `trafilatura-alpha/src/index.ts` (exported functions, classes, types, options interfaces). This drives every extraction and classification surface documented in the package README.
- **Live-crawl-tester** — `tools/live-crawl-tester/src/index.ts` (or its main entry) plus its CLI/option surface. It is a separate workspace package: a polite live-site fetcher (robots.txt, rate limit, disk cache) that runs extraction + classification over real URLs. It is NOT Crawlee/Playwright.
...
- Every exported library symbol (name, kind, signature, JSDoc) — note that it lives in **`trafilatura-alpha/src/index.ts`**.
- Every live-crawl-tester option/flag (name, type, default, help text) — note that it lives in **`tools/live-crawl-tester/src/`**.
...
- `README.md` (repo root)
- `trafilatura-alpha/README.md` (npm package page — alpha)
- `tools/live-crawl-tester/README.md`
- `training/README.md`
...
- The library API table (exports with signature and description) lives in `trafilatura-alpha/README.md`; the live-crawl-tester options live in `tools/live-crawl-tester/README.md`; the training workflow (dataset → `model.onnx` + `tfidf-vocab.json`) lives in `training/README.md`.
- The output-format / page-type label list — must equal the canonical set declared in `trafilatura-alpha/src/index.ts` everywhere it appears.
...
- Every exported library symbol appears in `trafilatura-alpha/README.md`.
- Every live-crawl-tester option appears in `tools/live-crawl-tester/README.md`.
- The training README's inputs/outputs match `training/`'s actual scripts (dataset → `model.onnx` + `tfidf-vocab.json`).
- The output-format / page-type label set in every README equals the canonical set in `trafilatura-alpha/src/index.ts`.
