...
**Scope:** This command only updates files inside the `htmlwasher` repo. The `packages/htmlwasher/README.md` is the published npm package page (alpha); keep it written for that audience. The root, training, and live-crawl-tester READMEs are repo-internal developer docs.
...
- **Library public API** — `packages/htmlwasher/src/index.ts` (exported functions, classes, types, options interfaces). This drives every extraction and classification surface documented in the package README.
...
- Every exported library symbol (name, kind, signature, JSDoc) — note that it lives in **`packages/htmlwasher/src/index.ts`**.
...
- `README.md` (repo root)
- `packages/htmlwasher/README.md` (npm package page — alpha)
...
- The library API table (exports with signature and description) lives in `packages/htmlwasher/README.md`; the live-crawl-tester options live in `packages/live-crawl-tester/README.md`; the training workflow (dataset → `model.xgb.json` + `tfidf-vocab.json`) lives in `training/README.md`.
- The output-format / page-type label list — must equal the canonical set declared in `packages/htmlwasher/src/index.ts` everywhere it appears.
...
- Every exported library symbol appears in `packages/htmlwasher/README.md`.
...
- The output-format / page-type label set in every README equals the canonical set in `packages/htmlwasher/src/index.ts`.
