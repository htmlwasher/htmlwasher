...
- The library API table (exports with signature and description) lives in `htmlwasher/README.md`; the live-crawl-tester options live in `tools/live-crawl-tester/README.md`; the training workflow (dataset → `model.onnx` + `tfidf-vocab.json`) lives in `training/README.md`.
