---
description: Root-cause the benchmark's per-type token-F1 gaps, apply DEFAULTS-ONLY fixes to trafilaturacore (Rust core / classifier / cleaning), re-run the offline gates after each fix, then re-benchmark and write a before/after timestamped report to @/benchmarks/.
...
Close benchmark gaps by fixing ROOT CAUSES in trafilaturacore — **never by tuning the benchmark, never by
...
- **Fidelity is reference-relative** (agreement with Trafilatura, not human gold); speed is internal
  extraction time; trafilaturacore does more (HTML out + classification + formatting), so its time/size are not
...
- **After EVERY fix, re-run the OFFLINE gates** (`pnpm test`, `cargo test --workspace`, the adbar eval, the
  clean-corpus-tester) so a live-page win never regresses the offline oracle. A live win that breaks the
  oracle is not a win.

## Step BASELINE

- Run the token benchmark exactly as `/bench:run` (its WIRE → BUILD → RUN → ANALYZE steps). Record the
  baseline: overall F1/P/R per engine, the per-type F1 (trafilaturacore vs Trafilatura AND vs rs-trafilatura),
  and the worst pages.
...
For each worst page, run `clean()` on its cached HTML (`~/r/trafilatura-external-tester/cache/<id>.html`, with
its `url` from `results.json`) and inspect `pageType` / `confidence` / `textLength` / the output — this
tells you which layer is at fault:

- **Rust core** — `@/packages/trafilaturacore/native/src/{extract,extractor/fallback,selector/*,html_processing,link_density}.rs`:
  over- or under-extraction, a missing structured rescue, a serializer issue.
- **Classifier** — `@/packages/trafilaturacore/native/src/page_type/*`: a wrong page type routes the wrong profile
  (the most common catastrophic under-extraction cause).
- **Cleaning** — `@/packages/trafilaturacore/src/cleaning/*`: over- or under-stripping in the sanitize/normalize pass.

Heuristics: a page where trafilaturacore "dropped real content" is usually over-aggressive boilerplate removal,
...
- Apply a defaults-only fix per root-caused gap via the domain agent (`Task`): `rust-pro` for the crate,
  `ts-pro` for cleaning/pipeline, `python-pro` for the classifier model/features. Give each agent the exact
...
- After EACH fix: `pnpm build && pnpm lint && pnpm test` (flagship + clean-corpus-tester) and
  `cargo test --workspace && cargo clippy --workspace --all-targets -- -D warnings && cargo fmt --check`,
  plus the adbar eval floors (`packages/trafilaturacore/test/validation/`). If any offline gate regresses,
  revert or refine the fix before continuing.

## Step REBENCH

- Rebuild the trafilaturacore output + prebuild, then re-run `pnpm bench`. Compare per-type F1 before/after.
...
- Write a timestamped report to `@/benchmarks/<TS>-benchmark.md` (`TS = $(date '+%Y-%m-%d-%H%M')`) exactly
  ...
  **Changes** section naming each fix: its root cause, the layer it touched (Rust core / classifier /
  cleaning), the concrete page(s) it fixed, and its F1 effect. Keep the mandatory Explanations section.
- Record the run + the fixes it drove in `@/PORTING-NOTES.md`.

## Step COMMIT

- Commit the trafilaturacore improvements (crate / TS / training, plus the rebuilt committed prebuild) with a
