...
- `pipeline.ts` lazy-loads `@trafilaturacore/native` — never reintroduce an eager top-level import (it breaks `boilerplate:'clean-keep-boilerplate'`, metadata-only use, and the CLI on platforms without a loadable prebuild)
