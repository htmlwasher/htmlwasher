...
- **What deliberately stays TypeScript:** the public `clean()` API and types, the cleaning pillar (the Trafilatura-aligned default config, custom `CleanConfig`, security floor), the metadata sidecar, decode (`chardet`/`iconv-lite`), and the CLI. Do NOT port these to Rust.
...
- **Language split.** Rust owns boilerplate removal, the 3-stage page-type cascade, the 7 extraction profiles, and confidence. TypeScript owns the public `clean()` API, the Trafilatura-aligned default cleaning config + custom `CleanConfig` + security floor, the metadata sidecar, buffer decoding, and the CLI. Python owns training. Do not move cleaning or metadata into Rust; do not reimplement extraction in TS.
...
- **Public API is frozen.** (Amended 2026-07-07: cleaning levels removed, `none` renamed `clean-only` — see `@/prompts/2026-7-7-align-levels-with-trafilatura/prompt.md`.) `clean(html, options?) → Promise<{ html, messages, metadata?, pageType?, confidence? }>` with `boilerplate: 'precision' | 'balanced' | 'recall' | 'clean-only'` (default `balanced`), `config?: CleanConfig` (custom JSON config; replaces the default Trafilatura-aligned config), `minify?`, `maxInputBytes?` (default 10 MB, enforced in TS **before** the FFI call), `url?` (context only — never fetched). Plain string unions, no TS `enum`s; **no `includeComments`/`includeTables`/`includeImages`/`includeLinks` toggles** — the cleaning config is the only content-inclusion control, and comments follow the classified page type. `boilerplate: 'clean-only'` never calls the Rust core and omits `pageType`/`confidence`. `DEFAULT_CLEAN_CONFIG` (the single Trafilatura-aligned cleaning config), the custom-config semantics, the security floor (`<script>` tag + content, every `on*` handler, `javascript:`/`vbscript:`/untrusted `data:` URLs, and dangerous inline CSS — `expression()`, `-moz-binding`, `url(javascript:|data:)` — stripped on every path, default and custom config alike; CSS-URL allow-list whenever inline `style` is allowed), and the CLI surface (`-b/-c/-m/-u/--json/-o/-q`, file/stdin → stdout, offline) are all implemented — see `@/packages/trafilaturacore/SPEC.md` and context doc 08; do not change them. Boundary validation stays hand-written runtime guards (`isCleanConfig`/`isBoilerplateMode` — **no `zod`**); `clean()` throws `TypeError` on a malformed config and the CLI exits non-zero with a clear stderr message. The floor is **unconditional** (Phase FLOOR): `enforceSecurityFloor` + the CSS-URL cleaner run as the final cleaning pass on EVERY path — default and custom config — closing the `{ "allowedAttributes": { "*": ["*"] } }` bypass doc 09 proved (in v1 that config passes validation and emits `onclick` and `javascript:` CSS URLs).
  ...
  packages/
    trafilaturacore/                    # the flagship TS library (npm `trafilaturacore`, unscoped, published) —
      src/                         #   moved from @/trafilaturacore/ at Phase RESTRUCTURE; public surface unchanged
        cleaning/                   # KEPT: sanitize/normalize/format pipeline + default config + security floor
        metadata/                  # KEPT: metadata sidecar (gains its own small DOM-parse helper)
        pipeline.ts                # REWIRED: metadata → @trafilaturacore/native extract() → cleanHtml()
...
(Amended 2026-07-07: the five cleaning levels and `correct` were removed and `none` was renamed `clean-only` — see `@/prompts/2026-7-7-align-levels-with-trafilatura/prompt.md`; read the level-era wording below as "every path/combo".)
...
- `pipeline.ts`: `runBoilerplate` now maps `boilerplate` mode → `focus`, passes `url`, and calls `@trafilaturacore/native`'s `extract()` (a `workspace:*` dependency of the flagship); `'clean-only'` still skips it entirely; the empty-result warn-and-clean-whole-doc behavior is preserved. Delete `src/core/`, `src/classifier/`, `src/profiles/`; move the DOM-parse helper metadata needs into `src/metadata/`; drop `onnxruntime-node`/`onnxruntime-web` and now-unused deps (`linkedom`/`parse5` stay for metadata + cleaning).
...
Unchanged in role from v1 (it already exists and passes; Phase RESTRUCTURE moves it up from `tools/trafilaturacore/`): a separate offline TypeScript project running the saved-fixture corpus (≥ 3 fixtures per page type across all 7 types, multilingual/Czech included) through `clean()` across five label-keyed combos — `balanced`, `precision`, `recall`, `clean-only` (each on the default config) plus `balanced+styled-config` (a custom config adding `<style>` + `class`/`style` attributes so the CSS-URL allow-list stays exercised) — asserting non-empty output, the security floor, the styled-config-superset rule (`balanced+styled-config` keeps ≥ as many distinct tag names as `balanced` — same extraction input, superset allow-list), and page-type plausibility against the manifest (reference combo: `balanced`); readable summary + report file; non-zero exit on failure; **never hits the network**. v2's changes: it exercises the Rust core underneath, and its security asserts are hard on EVERY combo — keep it green, and remember it imports `trafilaturacore` from `dist/` (rebuild first; turbo's `pnpm test` handles ordering).
...
- **Non-goals:**
  ...
  - No granular content toggles — the cleaning config is the single inclusion control.
...
- [ ] Phase FLOOR landed: the TS security floor is unconditional, the wildcard-`CleanConfig` bypass is closed with regression tests, and the corpus tester hard-asserts security at every level including `correct` (level-era wording; since 2026-07-07 read as: for every combo).
...
- **Lazy-load the FFI.** An eager top-level `import '@trafilaturacore/native'` made even `boilerplate:'clean-only'`, metadata-only, and CLI paths crash on platforms without a loadable binding. `pipeline.ts` now lazy-imports on the first extraction call and degrades to whole-document cleaning (with a warning) on any native failure; native `warnings` surface in `clean().messages` (`boilerplate:` prefix).
