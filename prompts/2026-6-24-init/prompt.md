    ...
    htmlwasher/                    # the flagship TS library (npm `htmlwasher`, unscoped, published) —
      ...
      native/                      # NEW: crate `htmlwasher-native` = npm `@htmlwasher/native` (private):
        src/                       #   the simplified rs-trafilatura fork — extraction pipeline, selectors,
                                   ...
                                   #   GBDT eval), serializer (HTML + text twin), #[napi] extract/extractSync
...
- **Gate:** `cargo test --workspace` green; the v1 adbar sanity-harness assertions (`packages/htmlwasher/test/adbar-corpus.test.ts`: minimum text length, expected content needles present, no `<script>`/`on*`/`class`/`style` in output) pass as a cargo test over the same cached pages; `favor_precision`/`favor_recall`/profile selection observably change output; the `html-cleaning` license is verified compatible.
...
- [ ] `packages/htmlwasher/native/`: the simplified rs-trafilatura fork — live-path extraction, 3-stage page-type cascade, 189-feature extractor, pure-Rust GBDT evaluator over the XGBoost JSON dump, 7 profiles, confidence; embedded validated artifacts; no network, no panics on malformed HTML; `cargo test` + `clippy` green.
