...
(htmlwasher vs Trafilatura vs rs-trafilatura vs mozilla/readability), produced by:
...
- `/bench:improve` — root-cause the per-type token-F1 gaps, apply defaults-only fixes to
  htmlwasher, re-run the offline gates, re-benchmark, and write a before/after report here.

The benchmark itself lives OUTSIDE this repo at `~/r/htmlwasher-external-tester/` — it is
external by design because it hits the network (fetching a cached corpus of real pages);
htmlwasher itself never touches the network. Fidelity is reference-relative (token
