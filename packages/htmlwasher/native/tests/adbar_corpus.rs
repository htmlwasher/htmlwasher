...
//! lives OUTSIDE this repo (`~/r/htmlwasher-sources/`), so the test skips gracefully
//! when absent (stays green in CI).
//!
//! RE-BASELINED for preserve-markup: the hygiene guarantee on the default emit is only
//! that `<script` is absent (class/style/attrs are preserved — TS washing sanitizes).
...
fn corpus_dir() -> Option<PathBuf> {
    ...
    let dir = PathBuf::from(home).join("r/htmlwasher-sources/trafilatura/tests/cache");
