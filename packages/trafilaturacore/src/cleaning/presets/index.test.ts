...
import { getCleanConfig } from './index.js';
...
describe('getCleanConfig', () => {
  it('maps each sanitize level to its preset', () => {
    expect(getCleanConfig('minimal')).toBe(minimalSetup);
    expect(getCleanConfig('standard')).toBe(standardSetup);
    expect(getCleanConfig('permissive')).toBe(permissiveSetup);
    expect(getCleanConfig('styled')).toBe(styledSetup);
  });

  it('returns undefined for correct (normalize-only, no sanitize)', () => {
    expect(getCleanConfig('correct')).toBeUndefined();
  });
});
