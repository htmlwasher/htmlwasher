...
//! blocks the event loop) and `extractSync` to the `@trafilaturacore/native` package
...
/// otherwise UNSANITIZED — the TS cleaning stage owns sanitization). `confidence` is
