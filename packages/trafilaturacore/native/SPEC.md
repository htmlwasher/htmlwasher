# trafilaturacore-native — SPEC

The Rust boilerplate-removal + page-type extraction core for trafilaturacore v2. A simplified
fork of rs-trafilatura's LIVE extraction path, ported through the tested v1 TypeScript core
(`packages/trafilaturacore/src/core/*`) with the doc-09 divergences applied. **Rust owns boilerplate
removal + the extraction profiles; the TypeScript layer owns sanitization, metadata, and the
public `clean()` API.** Phase CRATE deliverable (plain lib; napi bindings arrive at Phase BIND).
...
## napi binding (Phase BIND) — `@trafilaturacore/native`
...
Packaging (contextractor committed-prebuild pattern): `package.json` (`@trafilaturacore/native`,
...
The adbar sanity test skips gracefully when `~/r/trafilatura-sources/trafilatura/tests/cache` is absent.
